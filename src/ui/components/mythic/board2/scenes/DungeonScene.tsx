...
export function DungeonScene(props: DungeonSceneProps) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-3 rounded-xl border border-emerald-200/30 bg-[linear-gradient(165deg,rgba(22,40,33,0.95),rgba(7,11,17,0.98))] p-3 text-emerald-50">
      ...
      <div
        className="relative min-h-[280px] flex-1 overflow-hidden rounded-lg border border-emerald-200/35 bg-[radial-gradient(circle_at_50%_10%,rgba(74,222,128,0.18),rgba(2,8,14,0.95))]"
        onClick={(event) => {
      ...
      >
        ...
        {props.scene.hotspots.map((hotspot) => (
          <button
            key={hotspot.id}
            type="button"
            className="absolute rounded-md border border-emerald-200/45 bg-emerald-100/10 px-2 py-1 text-left text-[11px] text-emerald-100 shadow-[0_0_0_1px_rgba(0,0,0,0.2)] transition hover:bg-emerald-100/20"
            style={{
          ...
          >
            ...
            {hotspot.subtitle ? <div className="truncate text-[10px] text-emerald-100/75">{hotspot.subtitle}</div> : null}
          </button>
        ))}
      </div>

      <div className="grid gap-2 text-[11px] text-emerald-100/80 sm:grid-cols-2">
