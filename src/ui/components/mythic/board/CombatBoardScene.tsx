...
function hasBlockedLine(args: {
  ...
  const x0 = Math.floor(args.fromX);
  const y0 = Math.floor(args.fromY);
  const x1 = Math.floor(args.toX);
  const y1 = Math.floor(args.toY);
  ...
  while (!(x === x1 && y === y1)) {
    ...
    if (!isEndpoint && args.blocked.has(`${x},${y}`)) return true;
    const e2 = 2 * err;
    if (e2 > -dy) {
      err -= dy;
      x += sx;
...
export function CombatBoardScene(props: CombatBoardSceneProps) {
  ...
  const playerCombatant = useMemo(
    () => (props.playerCombatantId ? props.combatants.find((entry) => entry.id === props.playerCombatantId) ?? null : null),
    [props.combatants, props.playerCombatantId],
  );
  const blockedTileSet = useMemo(() => {
    const set = new Set<string>();
    for (const tile of props.blockedTiles ?? []) {
      set.add(`${Math.floor(tile.x)},${Math.floor(tile.y)}`);
    }
    return set;
  }, [props.blockedTiles]);
  const focusedLineBlocked = useMemo(() => {
    if (!playerCombatant || !focusedCombatant || playerCombatant.id === focusedCombatant.id) return false;
    return hasBlockedLine({
      fromX: playerCombatant.x,
      fromY: playerCombatant.y,
      toX: focusedCombatant.x,
      toY: focusedCombatant.y,
      blocked: blockedTileSet,
    });
  }, [blockedTileSet, focusedCombatant, playerCombatant]);

  const quickCastSkills = useMemo(() => {
    const byId = new Map(props.skills.map((skill) => [skill.id, skill] as const));
    return props.skillAvailability
      .map((entry) => ({ entry, skill: byId.get(entry.skillId) ?? null }))
      .filter((item): item is { entry: SkillAvailabilityEntry; skill: CombatBoardSkillLite } => Boolean(item.skill));
  }, [props.skillAvailability, props.skills]);

  const castFocusedSkill = async (entry: SkillAvailabilityEntry, skill: CombatBoardSkillLite) => {
    if (!props.playerCombatantId || props.isActing || props.isTicking) return;
    if (!entry.usableNow) return;
    const target: Target =
      skill.targeting === "self"
        ? { kind: "self" }
        : skill.targeting === "single"
          ? focusedCombatant
            ? { kind: "combatant", combatant_id: focusedCombatant.id }
            : { kind: "self" }
          : focusedCombatant
            ? { kind: "tile", x: Math.floor(focusedCombatant.x), y: Math.floor(focusedCombatant.y) }
            : { kind: "self" };
    setCastingSkillId(entry.skillId);
    try {
      if (props.onQuickCast) {
        await props.onQuickCast({ skillId: entry.skillId, target });
      } else {
        await props.onUseSkill({
          actorCombatantId: props.playerCombatantId,
  ...
  return (
    <div className="relative h-full min-h-[520px] overflow-hidden rounded-xl border border-red-200/20 bg-[linear-gradient(180deg,rgba(20,8,10,0.95),rgba(9,8,14,0.98))] p-3">
          ...
          onDraw={(ctx, frame) => {
            for (let y = 0; y < grid.h; y += 1) {
              for (let x = 0; x < grid.w; x += 1) {
                ...
                if (blockedTileSet.has(`${x},${y}`)) {
                  drawPixelRect(ctx, x * 10 + 1, y * 10 + 1, 8, 8, "rgba(101,54,61,0.88)");
                  drawPixelRect(ctx, x * 10 + 2, y * 10 + 2, 2, 2, "rgba(242,197,107,0.5)");
                  drawPixelRect(ctx, x * 10 + 6, y * 10 + 6, 2, 2, "rgba(242,197,107,0.35)");
                  drawOutlineRect(ctx, x * 10 + 1, y * 10 + 1, 8, 8, "rgba(0,0,0,0)", "rgba(242,197,107,0.45)");
                }
              }
            }
            ...
            if (playerCombatant && focusedCombatant && playerCombatant.id !== focusedCombatant.id) {
              const sx = Math.floor(playerCombatant.x) * 10 + 5;
              const sy = Math.floor(playerCombatant.y) * 10 + 5;
              const tx = Math.floor(focusedCombatant.x) * 10 + 5;
              const ty = Math.floor(focusedCombatant.y) * 10 + 5;
              ctx.beginPath();
              ctx.moveTo(sx + 0.5, sy + 0.5);
              ctx.lineTo(tx + 0.5, ty + 0.5);
              ctx.strokeStyle = focusedLineBlocked ? "rgba(239,107,107,0.9)" : "rgba(242,197,107,0.9)";
              ctx.lineWidth = 1;
              ctx.stroke();
            }
            ...
            if (hit) {
              ...
              props.onInspect({
                ...
                actions: [
                  ...
                  {
                    id: `combatant-focus-skill-tray:${hit.combatant.id}`,
                    label: "Use Skill on Target",
                    intent: "focus_target",
                    payload: { target_combatant_id: hit.combatant.id },
                  },
                  {
                  ...
                  {
                    id: `combatant-status-check:${hit.combatant.id}`,
                    label: "Status Check",
                    intent: "dm_prompt",
                    prompt: `Give me a concise combat status check focused on ${hit.combatant.name}.`,
                    payload: { target_combatant_id: hit.combatant.id, action: "status_check" },
                  },
                ],
                meta: {
      ...
      <div className="mt-2 grid grid-cols-1 gap-2 text-[11px] text-red-100/78 md:grid-cols-2">
        <div className="rounded border border-red-200/20 bg-red-100/10 px-2 py-1">
          ...
          {focusedCombatant && playerCombatant && focusedCombatant.id !== playerCombatant.id ? (
            <span className={`ml-2 ${focusedLineBlocked ? "text-red-200" : "text-emerald-200"}`}>
              LOS: {focusedLineBlocked ? "blocked" : "clear"}
            </span>
          ) : null}
        </div>
        <div className="rounded border border-red-200/20 bg-red-100/10 px-2 py-1">
      ...
      <div className="mt-2 rounded border border-red-200/25 bg-red-100/10 p-2">
        <div className="mb-2 text-[11px] font-medium text-red-100">
          {focusedCombatant
            ? `Quick Cast on ${focusedCombatant.name}`
            : "Select a target to quick cast"}
        </div>
        {quickCastSkills.length === 0 ? (
          <div className="text-[11px] text-red-100/70">No equipped active skills loaded.</div>
        ) : (
          <div className="flex flex-wrap gap-2">
            {quickCastSkills.map(({ entry, skill }) => {
              const disabled = !entry.usableNow || props.isActing || props.isTicking || !props.playerCombatantId;
              ...
              return (
                <button
                  ...
                  className={`rounded border px-2 py-1 text-[11px] transition ${
                    disabled
                      ? "cursor-not-allowed border-red-200/20 bg-red-100/10 text-red-100/50"
                      : "border-amber-200/45 bg-amber-200/15 text-amber-50 hover:bg-amber-200/25"
                  }`}
                  onClick={() => void castFocusedSkill(entry, skill)}
                  disabled={disabled}
                  title={reason ?? `${skill.name} · range ${entry.rangeTiles}`}
                >
                  <span className="font-medium">{skill.name}</span>
                  <span className="ml-1 text-[10px] text-red-100/70">
                    {castingSkillId === entry.skillId
                      ? "Casting..."
                      : reason
                        ? reason
                        : `r${entry.rangeTiles} cd${entry.cooldownTurns}`}
                  </span>
                </button>
              );
            })}
