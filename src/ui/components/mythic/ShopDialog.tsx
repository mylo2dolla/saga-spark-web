...
export function ShopDialog(props: {
    ...
    void (async () => {
      try {
        const { data, error: edgeError } = await callEdgeFunction<ShopStockPayload>("mythic-shop-stock", {
          ...
          timeoutMs: 25_000,
          maxRetries: 0,
          idempotencyKey: `shop-stock:${props.campaignId}:${props.vendorId}`,
          body: {
        ...
        if (e instanceof EdgeFunctionError) {
          ...
          setError(e instanceof Error ? e.message : "Failed to load shop stock");
        }
      } finally {
  ...
  }, [props.campaignId, props.open, props.vendorId]);
