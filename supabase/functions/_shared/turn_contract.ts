...
import { assertContentAllowed } from "./content_policy.ts";
...
export function parseDmNarratorOutput(rawText: string): { ok: true; value: DmNarratorOutput } | { ok: false; errors: string[] } {
  ...
  // Content policy validation on text fields. This is enforced server-side so the client never has to guess.
  try {
    ...
    return {
      ok: false,
      errors: [`content_policy:${error instanceof Error ? error.message : String(error)}`],
    };
  }
...
export function normalizeWorldPatches(raw: unknown): { patches: WorldPatch[]; dropped: number } {
  ...
  for (const entry of arr) {
    ...
    if (op === "FACT_CREATE" || op === "FACT_SUPERSEDE") {
      normalized.fact_key = typeof rec.fact_key === "string" ? rec.fact_key : typeof rec.key === "string" ? rec.key : "";
      normalized.data = asRecord(rec.data) ?? asRecord(rec.fact_json) ?? {};
    } else if (op === "ENTITY_UPSERT") {
      normalized.entity_key = typeof rec.entity_key === "string" ? rec.entity_key : typeof rec.id === "string" ? rec.id : "";
      normalized.entity_type = typeof rec.entity_type === "string" ? rec.entity_type : typeof rec.kind === "string" ? rec.kind : "entity";
      normalized.data = asRecord(rec.data) ?? asRecord(rec.entity_json) ?? {};
      normalized.tags = Array.isArray(rec.tags) ? rec.tags : [];
    } else if (op === "REL_SET") {
      normalized.subject_key = typeof rec.subject_key === "string" ? rec.subject_key : typeof rec.subject === "string" ? rec.subject : "";
      normalized.object_key = typeof rec.object_key === "string" ? rec.object_key : typeof rec.object === "string" ? rec.object : "";
      normalized.rel_type = typeof rec.rel_type === "string" ? rec.rel_type : typeof rec.type_name === "string" ? rec.type_name : "";
      normalized.data = asRecord(rec.data) ?? asRecord(rec.rel_json) ?? {};
    } else if (op === "QUEST_UPSERT") {
      normalized.quest_key = typeof rec.quest_key === "string" ? rec.quest_key : typeof rec.id === "string" ? rec.id : "";
