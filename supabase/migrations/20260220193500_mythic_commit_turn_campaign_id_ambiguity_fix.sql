-- Fix plpgsql column/argument ambiguity in mythic_commit_turn.
-- The function argument `campaign_id` conflicted with `campaign_runtime.campaign_id`
-- in an unqualified WHERE clause during turn commit.
...
begin
  ...
  select *
   ...
   where cr.campaign_id = mythic_commit_turn.campaign_id
   for update;
