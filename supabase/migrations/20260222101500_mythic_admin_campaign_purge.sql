...
as $$
begin
  if tg_op = 'DELETE' and current_setting('mythic.allow_append_delete', true) = '1' then
    return old;
  end if;
...
revoke all on function mythic.admin_purge_campaigns(uuid[]) from public;
grant execute on function mythic.admin_purge_campaigns(uuid[]) to service_role;
